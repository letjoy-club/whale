...
func (r *cityToTopicMatchingResolver) City(ctx context.Context, obj *models.CityToTopicMatching) (*models.Area, error) {
...
func (r *mutationResolver) UpdateMatching(ctx context.Context, matchingID string, param models.UpdateMatchingParam) (*models.Matching, error) {
	...
	if param.StartMatchingAt != nil {
		fields = append(fields, Matching.StartMatchingAt.Value(*param.StartMatchingAt))
	}
...
func (r *mutationResolver) UpdateMatchingDurationConstraint(ctx context.Context, userID string, param models.UpdateMatchingDurationConstraintParam) (string, error) {
	...
	midacontext.GetLoader[loader.Loader](ctx).MatchingDurationConstraint.Clear(ctx, constraint.UserID)
	return "", nil
}
...
// TopicDistribution is the resolver for the topicDistribution field.
func (r *queryResolver) TopicDistribution(ctx context.Context) ([]*models.TopicToMatching, error) {
	token := midacontext.GetClientToken(ctx)
	if !token.IsAdmin() {
		return nil, midacode.ErrNotPermitted
	}
	...
	topic2Matchings := allMatchingLoader.GetTopicToMatchings()

	ret := []*models.TopicToMatching{}
	for topicID, topic2Matching := range topic2Matchings {
		ret = append(ret, &models.TopicToMatching{
			TopicID: topicID,
			MatchingIds: lo.Map(topic2Matching, func(matching *models.Matching, i int) string {
				return matching.ID
			}),
		})
	}
...
// CityDistribution is the resolver for the cityDistribution field.
func (r *queryResolver) CityDistribution(ctx context.Context) ([]*models.CityToTopicMatching, error) {
	token := midacontext.GetClientToken(ctx)
	if !token.IsAdmin() {
		return nil, midacode.ErrNotPermitted
	}
	...
	allMatchingLoader := midacontext.GetLoader[loader.Loader](ctx).AllMatching
	allMatchingLoader.Load(ctx)
	city2Matchings := allMatchingLoader.GetCityToMatchings()

	ret := []*models.CityToTopicMatching{}
	for cityID, topicMatchings := range city2Matchings {
		m := []*models.TopicToMatching{}
		for topicID, topic2Matching := range topicMatchings {
			m = append(m, &models.TopicToMatching{
				TopicID: topicID,
				MatchingIds: lo.Map(topic2Matching, func(matching *models.Matching, i int) string {
					return matching.ID
				}),
		...
		ret = append(ret, &models.CityToTopicMatching{
			CityID: cityID,
			Topics: m,
		})
	}
...
// MatchingDurationConstraints is the resolver for the matchingDurationConstraints field.
func (r *queryResolver) MatchingDurationConstraints(ctx context.Context, userID string) (*models.MatchingDurationConstraint, error) {
	thunk := midacontext.GetLoader[loader.Loader](ctx).MatchingDurationConstraint.Load(ctx, userID)
	return thunk()
}

// Topic is the resolver for the topic field.
func (r *topicToMatchingResolver) Topic(ctx context.Context, obj *models.TopicToMatching) (*models.Topic, error) {
	return &models.Topic{ID: obj.TopicID}, nil
}

// CityToTopicMatching returns CityToTopicMatchingResolver implementation.
func (r *Resolver) CityToTopicMatching() CityToTopicMatchingResolver {
	return &cityToTopicMatchingResolver{r}
}

// TopicToMatching returns TopicToMatchingResolver implementation.
func (r *Resolver) TopicToMatching() TopicToMatchingResolver { return &topicToMatchingResolver{r} }

type cityToTopicMatchingResolver struct{ *Resolver }
type topicToMatchingResolver struct{ *Resolver }
